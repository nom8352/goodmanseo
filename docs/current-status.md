...
- 2026-08-13 daily AI key-news series: added `/blog/ai-keynews-2026-08-13/` with six practical, general-user AI items, a matching `1080x1350` share image, original-source links, a download button, static metadata, and sitemap registration. X is a discovery source; claims must be confirmed through company sources or reputable reporting. Prioritize launches people can try, free/price/usage changes, availability, major service changes, and broadly relevant company news. Avoid research papers, benchmark-only news, infrastructure detail, and enterprise statistics unless the user impact is clear. The deterministic image renderer is `scripts/generate-ai-keynews-image.mjs` and accepts 3–6 strong items so weak filler is unnecessary. X and KakaoTalk posting remain manual and outside the site workflow.
