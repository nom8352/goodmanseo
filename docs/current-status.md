...
- 2026-08-13 daily AI key-news series: added `/blog/ai-keynews-2026-08-13/` with five current items in a user-first mix, a matching `1080x1350` share image, original-source links, a download button, static metadata, and sitemap registration. Each edition should lead with 3–4 immediate user changes such as availability, pricing, usage limits, privacy, or account settings, then add 2–3 broadly important AI company or industry stories. X is a discovery source; claims must be confirmed through company sources or reputable reporting, and freshness must not be sacrificed to force a user-focused item. Avoid benchmark-only and infrastructure detail unless the wider impact is clear. The deterministic image renderer is `scripts/generate-ai-keynews-image.mjs` and accepts 3–6 strong items. X and KakaoTalk posting remain manual and outside the site workflow.
